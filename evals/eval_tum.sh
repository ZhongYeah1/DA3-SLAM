...
model_name=${2:-da3-small} # Default to da3-small if not provided
dataset_path="${abs_dir%/}/MASt3R-SLAM/datasets/tum/"
gt_path="${abs_dir%/}/MASt3R-SLAM/datasets/tum/"
log_path="$(pwd)/logs/tum_results_w${submap_size}_${model_name}.txt"
...
for run in $(seq 1 $n); do
    ...
    for dataset in "${datasets[@]}"; do
        echo "Running main.py on $dataset (Run $run) with model $model_name"
        dataset_name="${dataset_path}${dataset}/rgb"
        XFORMERS_DISABLED=1 python main.py --image_folder "$dataset_name" --max_loops 1 --min_disparity 50 --conf_threshold 25 --lc_thres 0.95 --submap_size "$submap_size" --model_name "$model_name" --log_results --skip_dense_log --log_path "$(pwd)/logs/${dataset}_run${run}_w${submap_size}_${model_name}.txt"
    done

    for dataset in "${datasets[@]}"; do
        echo "Evaluating $dataset (Run $run)"
        est_path="$(pwd)/logs/${dataset}_run${run}_w${submap_size}_${model_name}.txt"
